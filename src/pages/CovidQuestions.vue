    ...
    <div class="w-full h-auto flex justify-between">
      <Form @submit="onSubmit" v-slot="{ meta }" class="w-[600px] mt-11">
        ...
        <div v-if="covid === 'yes'" class="mt-11">
          <label-component :label="'antibody'" :title="'ანტისხეულების ტესტი გაქვს გაკეთებული?*'" />
          <div>
            <radio-input
              :rules="'required'"
              ...
              value="yes"
            />
            <radio-input v-model="antibody_test" id="antibody_test" labelValue="არა" value="no" />
          </div>
        </div>
        <ErrorMessage class="text-[#F15524] my-4" :name="'antibody_test'" />
        <div v-if="antibody_test === 'no' && covid === 'yes'" class="mt-11 h-22 flex flex-col">
          <label-component
            :label="'antibody'"
        ...
        <div v-if="antibody_test === 'yes' && covid === 'yes'" class="mt-11 flex flex-col">
          <label-component
            :label="'antibody'"
